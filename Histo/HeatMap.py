...
df = pd.read_excel(r'//equipe2-nas1/Gilles.DELBECQ/Data/Microscopie/Histo électrodes/Injections Chr2 retro/0002_full_res/plot data/data.xlsx',index_col=0)
# df = df.set_index('x')
...
plt.contour(np.arange(.5, norm_df.shape[1]), np.arange(.5, norm_df.shape[0]), norm_df, levels=[520], colors='yellow')
#plt.xlim([0,3000])
# plt.ylim([1000,1500])
#plt.gca().invert_yaxis()
plt.show()
