...
savefig_path=r'\\equipe2-nas1\Gilles.DELBECQ\Data\MOCAP\Cohorte 2\Figs\Length_steps_med_gap_filled/'
            ...
            if int(data_MOCAP.session_idx()) in sessions_to_analyze: #Takes only first 2 sessions (basic locomotion)
                ...
                for i in left_peaks:
                    if info[0] != "Off": #If it is a stim on trial
                        ...
                        Length_before_left.append(flat_coords[f"{idx[0]}:Left_Foot_Z"][i])
                        plt.plot(flat_coords[f"{idx[0]}:Left_Foot_X"][i], 
                                 flat_coords[f"{idx[0]}:Left_Foot_Z"][i], "o",c='red')
                        
                
                #Find the median Length for left and right steps to substract it later to each Length before beam 
                med_left = stat.median(Length_before_left)
                
                
                # Length_list_left.append([idx,Length_before_left,Length_after_left])
                Length_list_left.append([idx,Length_before_left-med_left,Length_after_left-med_left])
                ...
                for i in right_peaks:
                    if info[0] != "Off":#If it is a stim on trial
                        ...
                        if flat_coords[f"{idx[0]}:Right_Foot_X"][i] >=beam:#Before the beam
                            Length_before_right.append(
                                flat_coords[f"{idx[0]}:Right_Foot_Z"][i])
                            plt.plot(flat_coords[f"{idx[0]}:Right_Foot_X"][i], 
                                     flat_coords[f"{idx[0]}:Right_Foot_Z"][i], "o",c='blue')
                        else:#After the beam
                            Length_after_right.append(
                                flat_coords[f"{idx[0]}:Right_Foot_Z"][i])
                            plt.plot(flat_coords[f"{idx[0]}:Right_Foot_X"][i], 
                                     flat_coords[f"{idx[0]}:Right_Foot_Z"][i], "o",c='cyan')
                    else:#It is not a stim trial
                        plt.title(f'Length step detection {data_MOCAP.subject()}_{data_MOCAP.session_idx()}_{data_MOCAP.trial_idx()} Stim Off')
                        Length_before_right.append(flat_coords[f"{idx[0]}:Right_Foot_Z"][i])
                        plt.plot(flat_coords[f"{idx[0]}:Right_Foot_X"][i], 
                                 flat_coords[f"{idx[0]}:Right_Foot_Z"][i], "o",c='blue')
                
                #Find the median Length for left and right steps to substract it later to each Length before beam
                med_right = stat.median(Length_before_right)
                
                # Length_list_right.append([idx,Length_before_right,Length_after_right])
                Length_list_right.append([idx,Length_before_right-med_right,Length_after_right-med_right])
