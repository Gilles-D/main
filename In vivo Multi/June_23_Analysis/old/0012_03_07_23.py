...
for file in Files:
    ...
    print(f'{data_MOCAP.subject()}_{data_MOCAP.session_idx()}_{data_MOCAP.trial_idx()}')
    ...
    left_foot_peaks = peaks(data_MOCAP.coord(f"{data_MOCAP.subject()}:Left_Foot")[0],data_MOCAP.coord(f"{data_MOCAP.subject()}:Left_Foot")[1],data_MOCAP.coord(f"{data_MOCAP.subject()}:Left_Foot")[2])
    right_foot_peaks = peaks(data_MOCAP.coord(f"{data_MOCAP.subject()}:Right_Foot")[0],data_MOCAP.coord(f"{data_MOCAP.subject()}:Right_Foot")[1],data_MOCAP.coord(f"{data_MOCAP.subject()}:Right_Foot")[2])
    ...
    #Exclure les cycles plus de 3 fois plus long que la moyenne des cycles
    # Calcul de la durée moyenne des cycles
    mean_duration = np.mean([len(event) for event in slices_all])
    
    # Seuil pour exclure les cycles plus longs
    threshold = 3 * mean_duration
    
    # Filtrage des cycles pour ne garder que ceux dont la durée est inférieure au seuil
    slices = [event for event in slices_all if len(event) <= threshold]
    
    print(rf'{len(slices_all) - len(slices)} cycles exclus')

    #Centrage et alignement des cycles
    max_duration = max(len(event) for event in slices)
    # Liste pour stocker les événements alignés et étirés
    aligned_events = [] 

    # Boucle pour aligner et étirer chaque événement
    for event in slices:
        aligned_event = np.interp(np.linspace(0, 1, max_duration), np.linspace(0, 1, len(event)), event)
    ...
    plt.figure()
    plt.title(rf"Cycles {data_MOCAP.subject()}_{data_MOCAP.session_idx()}_{data_MOCAP.trial_idx()}")
    # Tracé des événements alignés et étirés
    for i, aligned_event in enumerate(aligned_events):
        # Calcul de l'axe x en pourcentage
        x = np.linspace(0, 100, len(aligned_event))
    
        # Traçage de l'événement avec l'axe x en pourcentage
        plt.plot(x, aligned_event, label=f'Événement {i+1}', color='grey')
        
    plt.plot(x,mean_event)
    
    # Ajout des labels et de la légende
    plt.xlabel('Pourcentage de la durée')
    plt.ylabel('Valeur de l\'événement')
    plt.legend()
...
for i in range(len(mocap_starts)):
    try:
        ...
        print(rf'{i} out of lengths list range')



for index,trial in enumerate(mocap_trials_spikes):
    print(rf'Trial {index+1}')
    mocap_trial_elephant_spiketrains = []
    ...
    for i,spiketrain in enumerate(mocap_trial_elephant_spiketrains):
        ...
        try:
            ...
            inst_rate_norm= np.divide(inst_rate, max(inst_rate))
            time_axis = np.array(range(len(inst_rate_norm)))/sampling_rate*1000/5 #sampling period = 50 -> /1 ; sampling period = 10 -> /1 
            
        
            plt.plot(time_axis,inst_rate, label=rf'cluster {i+1}')
...
"""
Nicer loop
For each trial - Unified dataframe : 
    - spiking rate of each unit
    - mocap data
    - 
