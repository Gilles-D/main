...
def higher_channel_order_dict(recording_path,spikesorting_results_path, sorter_list, saving_spike_path, saving_waveform_path, saving_summary_plot, unit_base_nb=0, number_of_channel_for_comp=2,concatenated_signals=None,do_concatenate=True):
    ...
    for sorter in sorter_list:
        ...
        if sorter == 'comp_mult_2_kilosort3_mountainsort4_tridesclous':
            sorter_mini = 'comp'
...
def open_figures_fullscreen():
    """
    Ouvre toutes les figures existantes en plein écran.
    """
    # Obtenir toutes les figures ouvertes
    figures = [plt.figure(num) for num in plt.get_fignums()]
    ...
    for i, fig in enumerate(figures):
        ...
        # Définir la position et la taille de la figure
        fig.canvas.manager.window.setGeometry(0, 0, fig_width, fig_height)
...
#To bypass the loading of really long concatenated signal, load them splitted (be careful with the order)
concatenated_signals = [

    ]

higher_channel_order_dict(recording_path,spikesorting_results_path,sorter_list,saving_spike_path,saving_waveform_path,saving_summary_plot,concatenated_signals=concatenated_signals, do_concatenate=False)
