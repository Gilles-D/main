...
for layout_idx in range(num_layouts):
    ...
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(45, 60))
...
time_window = 1  # Time start of window before center in seconds
...
for Unit in unit_list :
    ...
    for i, behavior in enumerate(unique_behaviors):
        ...
        histo1 = ax1.hist([elt for lst in spikes_next for elt in lst], bins=np.arange(-time_window, time_window + 0.05, 0.05))
        max_histo1 = max(histo1[0])
        ax1.add_patch(Rectangle((0, 0), time_window, 0.1*max_histo1, color='black', alpha=0.5))
        
        ax2.hist([elt for lst in spikes_prev for elt in lst], bins=np.arange(-time_window, time_window + 0.05, 0.05))
        ax2.add_patch(Rectangle((-time_window, 0), time_window, 0.1*max_histo1, color='black', alpha=0.5))
...
DLC['speed_x-Tail_Base'].dropna().hist(bins=50, figsize=(16,20), xlabelsize=8, ylabelsize=8)
sns.kdeplot(data=DLC['speed_x-Tail_Base'], fill=True)

#calcule vitesse y_tail
DLC['speed_y-Tail_Base'].dropna().hist(bins=50, figsize=(16,20), xlabelsize=8, ylabelsize=8)
