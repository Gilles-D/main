...
# df_stim_scaled = StandardScaler().fit_transform(optoD[['Z-score', '% success', 'jitters_w']])
...
def generate_plot(x_column, y_column, color_column='', size_column='', enable_annotations=False):
    fig, ax = plt.subplots(figsize=(5, 5))
    
    if not color_column:
        color_data = 'black'
    else:
        color_data = um[color_column]
    
    if not size_column:
        # If no size column is specified, use a default size (e.g., 50)
        scaled_sizes = np.full(len(um), 50)        # Calculate the percentage of the max value for the specified size column

    else:
        max_size = um[size_column].max()
        scaled_sizes = (um[size_column] / max_size) * 100
        
    scatter = plt.scatter(um[x_column], um[y_column], c=color_data, s=scaled_sizes, cmap='viridis')
    
    plt.xlabel(x_column)
    plt.ylabel(y_column)
    plt.title(f'{x_column} vs. {y_column} (size: {size_column})')
    plt.colorbar(scatter, label=color_column)
    
    if enable_annotations:
        for index, row_data in um.iterrows():
            x = row_data[x_column]
            y = row_data[y_column]
            truncated_index = str(index)[5:]  # Remove the first 5 characters
            ax.annotate(str(truncated_index), (x, y), textcoords="offset points", xytext=(0, 5), fontsize=8)
...
def generate_2x2_plot(x_column, y_column, color_columns, size_column='', enable_annotations=False):
    ...
    for i, color_col in enumerate(color_columns):
        row, col = i // 2, i % 2
        scatter = axs[row, col].scatter(um[x_column], um[y_column], c=um[color_col], s=scaled_sizes, cmap='viridis')
        ...
        if enable_annotations:
            # Add annotations for each point in the first axis
            if row == 0 and col == 0:
                for index, row_data in um.iterrows():
                    x = row_data[x_column]
                    y = row_data[y_column]
                    truncated_index = str(index)[5:]  # Remove the first 5 characters
                    axs[row, col].annotate(str(truncated_index), (x, y), textcoords="offset points", xytext=(0, 5), fontsize=8)
...
def create_layout():
    return [
        ...
        [sg.Text("Select Size by column:"), sg.Combo(values=list(um.columns), key='size_col')],
...
while True:
    ...
    if event == "Generate Plot Individual":
        ...
        size_col = values['size_col']  # Size column
        enable_annotations = values['enable_annotations']  # Checkbox state
        generate_plot(x_col, y_col, color_col, size_col, enable_annotations)
    ...
    if event == "Generate All Plots":
        ...
        size_col = values['size_col']  # Size column
        enable_annotations = values['enable_annotations']  # Checkbox state        
        generate_2x2_plot(x_col, y_col, color_cols, size_col, enable_annotations)
...
fig, axes = plt.subplots(nrows=4, ncols=6, figsize=(30, 20), sharex=False, sharey=False)
...
fig6, axes = plt.subplots(nrows=4, ncols=6, figsize=(30, 20), sharex=False, sharey=False)
axes = axes.ravel()             # array to 1D
...
fig7, axes = plt.subplots(nrows=4, ncols=6, figsize=(30, 20), sharex=False, sharey=False)
axes = axes.ravel()             # array to 1D
cols = um_drop.columns[1:-1]     # create a list of dataframe columns to use
