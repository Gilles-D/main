...
#%%Parameters
session_name = '0022_28_07'
session_results = '0022_28_07'
...
for unit in unit_list[1:] :
    ...
    # Save the AUG to a file
    opto_name = os.path.join(save_dir, f"{unit}_stim_PSTH.png")
    plt.savefig(opto_name)
    ...
    # first_spikes_histo = [x for x in first_spikes if (not math.isnan(x) and x < 0.03)]
    # plt.hist(first_spikes_histo, bins=np.arange(0, 0.031, 0.002))
    # plt.title(f'first spikes {unit}')
    # plt.show()
