...
#Files location
data_dir = r'C:\Users\Gilles.DELBECQ\Desktop\17-05-21\smr'
...
for file_path in List_File_paths:
    ...
    #Load stim times
    stim_key = reader.get_event_timestamps(event_channel_index=1)
    stim_key = np.array(stim_key[0])
    stim_key_corrected=(stim_key/ 500000.00000000006)*sampling_rate
    stim_idx=[]
    for i in stim_key:
        stim_init=int(i+(50/1000)*500000.00000000006)
        stim_idx.append(stim_init)
        for n in range(99):
            stim_idx.append(stim_init+n*int((250/1000)*500000.00000000006))
    
    stim_idx_corrected=[]
    for i in stim_idx:
        stim_idx_corrected.append((i/500000.00000000006)*sampling_rate)
    
    stim_times=[]
    for i in stim_idx:
        stim_times.append(i/ 500000.00000000006)
    ...
    plt.savefig("{}\Spike Detection {}_neg.pdf".format(data_dir,exp))
    ...
    plt.savefig("{}\Raster+PSTH {}_neg.pdf".format(data_dir,exp))
