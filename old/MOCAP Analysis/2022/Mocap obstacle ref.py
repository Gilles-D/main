...
for idx in df_obstacle['ID'].tolist():
    ...
    for file in Files:
        ...
        if obstacle_idx == idx:
        # if obstacle_idx == 's8' or obstacle_idx == 's9' or obstacle_idx == 's10':
            obstacle = df_obstacle.loc[df_obstacle['ID'] == obstacle_idx]
            
            ref_x,ref_y,ref_z = obstacle['X'],obstacle['Y'],obstacle['Z']
            target_x, target_y,target_z = coord(file,"{}:Foot_R2".format(subject))
            norm_x=target_x-float(ref_x)
            norm_y=target_y-float(ref_y)
            norm_z=target_z-float(ref_z)
        
            plt.plot(-norm_y,norm_z,color='blue',label='right')
    figure.savefig(f'{root_dir}/Figs/{subject}_{idx}_obstacle centered.png')