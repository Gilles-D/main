# -*- coding: utf-8 -*-
"""
Created on Thu Apr 21 14:56:32 2022
...
for animal in animals:
    ...
    for file in Files:
        ...
        if data_MOCAP.subject() == animal:
            if int(data_MOCAP.session_idx()) in [1,2]: #Takes only first 2 sessions (basic locomotion)
                ...
                """
                ZSCORER LES LISTES DE Z[PEAK]
                """
                z_score_left=stats.zscore(flat_coords[f"{idx[0]}:Left_Foot_X"][left_peaks])
                z_score_right=stats.zscore(flat_coords[f"{idx[0]}:Right_Foot_X"][right_peaks])
                ...
                for i in left_peaks:
                    if math.isnan(info[0]) == False: #If it is a stim on trial
                        ...
                        height_before_left.append(z_score_left[t])
                        plt.plot(flat_coords[f"{idx[0]}:Left_Foot_X"][i], 
                                 flat_coords[f"{idx[0]}:Left_Foot_Z"][i], "o",c='red')
                    t=t+1        
                ...
                for i in right_peaks:
                    if math.isnan(info[0]) == False:#If it is a stim on trial
                        ...
                        plt.title(f'Height step detection {data_MOCAP.subject()}_{data_MOCAP.session_idx()}_{data_MOCAP.trial_idx()} Stim Off')
                        height_before_right.append(z_score_right[t])
                        plt.plot(flat_coords[f"{idx[0]}:Right_Foot_X"][i], 
                                 flat_coords[f"{idx[0]}:Right_Foot_Z"][i], "o",c='blue')
                    t=t+1
    ...
    figure1.savefig(f"{savefig_path}/Right_steps_{animal}_zscored.svg")
    figure1.savefig(f"{savefig_path}/Right_steps_{animal}_zscored.png")
    figure2.savefig(f"{savefig_path}/Left_steps_{animal}_zscored.svg")
    figure2.savefig(f"{savefig_path}/Left_steps_{animal}_zscored.png")