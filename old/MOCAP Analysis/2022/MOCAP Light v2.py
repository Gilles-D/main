...
for idx in df_obstacle['ID'].tolist():
    ...
    for file in Files:
        ...
        if obstacle_idx == idx:
            ...
            if freq == 'None':
                ...
                no_stim.append(np.asarray((-right_foot[1],right_foot[2])))
            else:
                plt.plot(-right_foot[1],right_foot[2],color='blue',label='Right',alpha=0.5)
                stim.append((-right_foot[1],right_foot[2]))
    ...
    # figure2.savefig(f'{root_dir}/Figs/{subject}_{idx}_effets stim full.png')
