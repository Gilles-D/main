...
#Files location
data_dir = r'H:/Seafile/Data/Tests PAD/05-03-21'
...
for file_path in List_File_paths:
    ...
    for i in range(0, len(stim_times)):
        signal_chunks.append(signal[int(stim_idx_corrected[i]) : int(stim_idx_corrected[i]) + int(window_length*sampling_rate)])
       
    signal_chunks_filtered = list()
    for i in range(0, len(stim_times)):
        signal_chunks_filtered.append(moving_average(signal[int(stim_idx_corrected[i]-99) : int(stim_idx_corrected[i]-99) + int(window_length*sampling_rate)]))
    ...
    plt.axvline(x=0.095, color='green')
